...
namespace HG.FFF.Old
{
    private static class NotInUsage
    {
        ...
        ILGenerator generator)
        {
                ...
                switch (landmarkSearchState)
                {
                    ...
                    case 2:
                        ...
                        if (list[i].opcode == OpCodes.Ldloc_S)
                        {
                            ...
                            {
                                Verse.Log.Error("<color=grey>[HG]</color> Failed to patch <color=#008080FF>Save Our Ship 2</color> EVAlevelSlow method: o_apparel mismatch");
                                return instructions;
                            }
            ...
            if (i_ENDFINALLY == 0)
            {
                Verse.Log.Error("<color=grey>[HG]</color> Unable to find all spacesuit check landmark instructions. Has <color=#008080FF>Save Our Ship 2</color> been updated?");
                return instructions;
            }
            /*else
            {
